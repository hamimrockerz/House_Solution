...
class _RenterProfilePageState extends State<RenterProfilePage> {
  ...
  Future<void> _loadProfileData() async {
    ...
    if (contact != null) {
      // Fetch renter information from Firebase
      ...
      if (renterEvent.snapshot.exists) {
        ...
        if (renters != null) {
          ...
          _selectedGender = renterData['gender']; // Fetch selected gender
          _maritalStatus = renterData['maritalStatus']; // Fetch marital status
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No renter information found for this contact.')),
        );
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return CustomScaffold(
      child: Container(
        // Removed the gap by adjusting padding
